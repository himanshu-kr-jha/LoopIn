...
const Sidebar = ({ onSelectUser }) => {
  ...
  //chats function
  const isOnline = nowOnline.map((userId) =>
  ...
  useEffect(() => {
    const chatUserHandler = async () => {
      setLoading(true);
      try {
        const chatters = await axios.get(`http://localhost:5001/api/user/currentchatters`,{
  ...
  const handelSearchSubmit = async (e) => {
    ...
    try {
      const search = await axios.get(
        `http://localhost:5001/api/user/search?search=${searchInput}`,{
          withCredentials: true,
        }
      );
                ...
                <div key={user._id}>
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-1">
                      <p className="font-bold text-black-950">{user?.name}</p>
                    </div>
                  </div>
